...
sub _listApacheAndUserFilesInFolder {
  ...
  while (my $entry = readdir($dh)) {
    ...
    push @entriesInfo, join("\t", HelpFuncs::encodePath($entry), $mode, $uid, $gid, $size, $mtime, $ctime);
  }
  closedir($dh);
  ...
  foreach my $entryInfo (@entriesInfo) {
    my ($entry, $mode, $uid, $gid, $size, $mtime, $ctime) = split("\t", $entryInfo);
    ...
    print $outFile join("\t", HelpFuncs::encodePath($entryRelPath), $ftype, $fsize, $mtime, ++$$orderNumber, $usersCache->{$uid}, $groupsCache->{$gid}, $mode) . "\n";
