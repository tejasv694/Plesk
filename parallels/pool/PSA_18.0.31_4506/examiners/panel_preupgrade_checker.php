...
define('PRE_UPGRADE_SCRIPT_VERSION', '18.0.31.0'); //script version
define('PLESK_VERSION', '18.0.31'); // latest Plesk version
...
class Plesk17KnownIssues
{
    function validate()
    {
        ...
        if (Util::isWindows()) {
            ...
            $this->checkDomainControllerLocation(); //:INFO: https://support.plesk.com/hc/articles/360007273353
        }
    }
    ...
    private function checkDomainControllerLocation()
    {
        ...
        if (preg_match("/IS_DOMAIN_CONTROLLER:\s*true/", $output)) {
            $log->warning('Active Directory Domain Controller and Plesk are on the same server. Read https://support.plesk.com/hc/articles/360007273353 for details.');
            $log->resultWarning();
        } else {
            $log->resultOk();
        }
    ...
    private function checkDomainsGuidCollation()
    {
        ...
        if (!empty($duplicates)) {
            $log->error(
                ...
                . "Please check https://support.plesk.com/hc/en-us/articles/360015662360 for more details."
            );
            $log->resultError();
        }
    }
