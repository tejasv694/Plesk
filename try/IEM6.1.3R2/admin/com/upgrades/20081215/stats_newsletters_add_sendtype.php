...
* @package SendStudio
*/
class stats_newsletters_add_sendtype extends Upgrade_API
{
	/**
	* RunUpgrade
	...
	*/
	function RunUpgrade()
	{
		if ($this->ColumnExists('stats_newsletters', 'sendtype')) {
			return true;
		}
		$query = 'ALTER TABLE ' . SENDSTUDIO_TABLEPREFIX . 'stats_newsletters ADD sendtype VARCHAR(100) default \'newsletter\'';
		$result = $this->Db->Query($query);
		if ($result) {
			$query = "UPDATE " . SENDSTUDIO_TABLEPREFIX . "stats_newsletters set sendtype='newsletter'";
			$result = $this->Db->Query($query);
		}
		return $result;
	}
