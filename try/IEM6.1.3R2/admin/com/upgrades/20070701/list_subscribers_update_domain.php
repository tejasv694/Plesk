...
* @package SendStudio
*/
class list_subscribers_update_domain extends Upgrade_API
{
	/**
	* RunUpgrade
	...
	*/
	function RunUpgrade()
	{
		if (!$this->ColumnExists('list_subscribers', 'domainname')) {
			return false;
		}

		if (SENDSTUDIO_DATABASE_TYPE == 'mysql') {
			$query = 'UPDATE ' . SENDSTUDIO_TABLEPREFIX . 'list_subscribers SET domainname=SUBSTRING(emailaddress FROM LOCATE(\'@\', emailaddress)) WHERE domainname IS NULL';
		}
		if (SENDSTUDIO_DATABASE_TYPE == 'pgsql') {
			$query = 'UPDATE ' . SENDSTUDIO_TABLEPREFIX . 'list_subscribers set domainname=SUBSTRING(emailaddress FROM POSITION(\'@\' IN emailaddress)) WHERE domainname IS NULL';
		}
