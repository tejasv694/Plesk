...
* @package SendStudio
*/
class users_change_textfooter extends Upgrade_API
{
	/**
	* RunUpgrade
	...
	*/
	function RunUpgrade()
	{
		if (!$this->ColumnExists('users', 'textfooter')) {
			return false;
		}

		if (SENDSTUDIO_DATABASE_TYPE == 'mysql') {
			$query = 'alter table ' . SENDSTUDIO_TABLEPREFIX . 'users change textfooter textfooter text';
		} else {
			$query = 'alter table ' . SENDSTUDIO_TABLEPREFIX . 'users alter column textfooter type text';
		}
