...
* @package SendStudio
*/
class users_change_htmlfooter extends Upgrade_API
{
	/**
	* RunUpgrade
	...
	*/
	function RunUpgrade()
	{
		if (!$this->ColumnExists('users', 'htmlfooter')) {
			return false;
		}

		if (SENDSTUDIO_DATABASE_TYPE == 'mysql') {
			$query = 'alter table ' . SENDSTUDIO_TABLEPREFIX . 'users change htmlfooter htmlfooter text';
		} else {
			$query = 'alter table ' . SENDSTUDIO_TABLEPREFIX . 'users alter column htmlfooter type text';
		}
