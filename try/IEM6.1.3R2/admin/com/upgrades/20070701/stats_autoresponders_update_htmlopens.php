...
* @package SendStudio
*/
class stats_autoresponders_update_htmlopens extends Upgrade_API
{
	/**
	* RunUpgrade
	...
	*/
	function RunUpgrade()
	{
		if (!$this->ColumnExists('stats_autoresponders', 'htmlopens')) {
			return false;
		}

		if (!$this->ColumnExists('stats_autoresponders', 'htmlopens_unique')) {
			return false;
		}

		if (!$this->ColumnExists('stats_autoresponders', 'textopens')) {
			return false;
		}

		if (!$this->ColumnExists('stats_autoresponders', 'textopens_unique')) {
			return false;
		}

		$query = 'update ' . SENDSTUDIO_TABLEPREFIX . 'stats_autoresponders set htmlopens=0, htmlopens_unique=0, textopens=0, textopens_unique=0 WHERE htmlopens IS NULL AND htmlopens_unique IS NULL AND textopens IS NULL AND textopens_unique IS NULL';
