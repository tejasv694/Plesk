...
(function($){
  ...
  $._timePicker = function(elm, settings) {
    ...
    var $tpDiv = $('<div class="time-picker'+ (settings.show24Hours ? '' : ' time-picker-12hours') +'" style="z-index:1024"></div>');
    var $tpList = $('<ul></ul>');
    ...
    var showPicker = function() {
	 var elmOffset = $(this).offset();
		// Append the timPicker to the body and position it.
		$tpDiv.css({'top':elmOffset.top+5, 'left':elmOffset.left});
    ...
    .blur(function() {
      if (!tpOver && $tpDiv[0].parentNode) { // Dont remove when timePicker is clicked or when already removed
        $tpDiv.hide();
      }
    })
    ...
    this.setTime = function(time) {
      ...
      // Trigger elements change events.
      $(elm).change();
    };
  ...
  function setTimeVal(elm, sel, $tpDiv, settings) {
    ...
    // Trigger elements change events.
    $(elm).change();
    // Keep focus for all but IE (which doesnt like it)
    if (!$.browser.msie) {
      elm.focus();
    }
    // Hide picker
